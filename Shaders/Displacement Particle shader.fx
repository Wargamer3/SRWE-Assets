...
VertexShaderOutput ParticleVertexShader(VertexShaderInput input)
{
    ...
    float2 tempPos = input.Position - Camera + input.Speed * NormalizedTimeAlive + Gravity * NormalizedTimeAlive;
    float2 FinalPos = tempPos + (input.UV * 2 - 1) * FinalSize;
	
    output.Position = mul(float4(FinalPos, 0, 1), ViewProjection);
...
{
    float4 ActualColor = SAMPLE_TEXTURE(Sampler, input.UV) * input.Color;
    float4 FinalColor = float4(input.UV.x, input.UV.y, 0, ActualColor.a);
    return FinalColor;
}
